...
public:
      ...
      goal_reached_published_(false)
    {
        ...
        // Subscriber for cmd_vel (to detect when robot stops moving)
        cmd_vel_sub_ = this->create_subscription<geometry_msgs::msg::Twist>(
            "/cmd_vel", 10,
            [this](const geometry_msgs::msg::Twist::SharedPtr) {
                last_cmd_vel_time_ = this->now();
                goal_reached_published_ = false; // reset if robot moves again
            });
        last_cmd_vel_time_ = this->now();
...
private:
    ...
    void sequence()
    {
        if (step_ == 0) {
            ...
            if (elapsed > 5.0 && !goal_reached_published_) {
                publish_status("Goal reached successfully! (cmd_vel silent >5s)");
                goal_reached_published_ = true;
            }
        }
        else if (step_ == 3) {
            // Brute force check: if no cmd_vel for 5s, assume goal reached
            auto elapsed = (this->now() - last_cmd_vel_time_).seconds();
            if (elapsed > 5.0 && !goal_reached_published_) {
                publish_status("Goal reached successfully! (cmd_vel silent >5s)");
                goal_reached_published_ = true;
